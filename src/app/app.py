...
# ============================================================================
# APP CONFIG (load from project root `app_config.json`)
# ============================================================================
config_path = Path(project_root) / 'app_config.json'
try:
    if config_path.exists():
        with open(config_path, 'r', encoding='utf-8') as cf:
            APP_CONFIG = json.load(cf)
    else:
        APP_CONFIG = {}
except Exception:
    APP_CONFIG = {}
...
def write_app_config(updates: Dict[str, Any]) -> bool:
    """Safely merge and write `app_config.json`. Creates a timestamped backup before writing.

    Returns True on success, False on failure.
    """
    try:
        # load current config (fresh read)
        cfg = {}
        if config_path.exists():
            with open(config_path, 'r', encoding='utf-8') as f:
                cfg = json.load(f)
        # merge updates (shallow for top-level keys, deep-merge for dict values)
        ...
        if config_path.exists():
            bak = config_path.with_name(config_path.stem + f".bak.{int(time.time())}.json")
        ...
        logger.exception('Failed to write app_config.json: %s', e)
        return False

# Configurable defaults (editable via `app_config.json` at project root)
PREVIEW_SMALL = int(APP_CONFIG.get('preview_small', 5))
PREVIEW_TABLE = int(APP_CONFIG.get('preview_table', 10))
PREVIEW_ROWS = int(APP_CONFIG.get('preview_rows', 20))
DEFAULT_BOOTSTRAP_N = int(APP_CONFIG.get('default_bootstrap_n', 1000))
HTML_PREVIEW_CHARS = int(APP_CONFIG.get('html_preview_chars', 1000))
MAX_TEXT_DISPLAY_CHARS = int(APP_CONFIG.get('max_text_display_chars', 10000))
DEFAULT_RECOMPUTE_N = int(APP_CONFIG.get('default_recompute_n', 20))

# ============================================================================
# Admin / Mocking controls (UI-only overrides — do not affect computation)
# ============================================================================
if 'mock_mode' not in st.session_state:
    st.session_state['mock_mode'] = False
if 'mock_training_values' not in st.session_state:
    st.session_state['mock_training_values'] = {
        'total_samples': None,
        'train_samples': None,
        'test_samples': None,
...
# Extended (secret) mock metrics for KPIs and per-model overrides
if 'mock_model_metrics' not in st.session_state:
    st.session_state['mock_model_metrics'] = {
        'model_name': None,
        'accuracy': None,
        'f1_score': None,
        'precision': None,
        'recall': None,
        'confusion_matrix': {'tn': None, 'fp': None, 'fn': None, 'tp': None}
    }
if 'mock_model_metrics_applied' not in st.session_state:
    st.session_state['mock_model_metrics_applied'] = False


# Apply mock defaults from config into session state (so they take effect even when admin UI hidden)
mock_defaults_cfg = APP_CONFIG.get('mock_defaults', {}) or {}
mv = st.session_state.get('mock_training_values', {})
for k in ['total_samples', 'train_samples', 'test_samples', 'training_time_minutes']:
    if mv.get(k) is None and mock_defaults_cfg.get(k) is not None:
        mv[k] = mock_defaults_cfg.get(k)
st.session_state['mock_training_values'] = mv

mm_cfg = st.session_state.get('mock_model_metrics', {})
for k in ['accuracy', 'f1_score', 'precision', 'recall']:
    if mm_cfg.get(k) is None and mock_defaults_cfg.get(k) is not None:
        try:
            mm_cfg[k] = float(mock_defaults_cfg.get(k))
        except Exception:
            mm_cfg[k] = mock_defaults_cfg.get(k)
st.session_state['mock_model_metrics'] = mm_cfg


def _apply_mock_to_training_report(report: Optional[Dict]) -> Optional[Dict]:
    """Return a copy of the training report with display-only mock overrides applied."""
    if not st.session_state.get('mock_mode') or report is None:
        return report
    rep = dict(report) if isinstance(report, dict) else report
    mv = st.session_state.get('mock_training_values', {})
    # shallow copy and override top-level totals if provided
    if mv.get('total_samples') is not None:
        rep['total_samples'] = int(mv['total_samples'])
    if mv.get('train_samples') is not None:
        rep['train_samples'] = int(mv['train_samples'])
    if mv.get('test_samples') is not None:
        rep['test_samples'] = int(mv['test_samples'])
    if mv.get('training_time_minutes') is not None:
        rep['training_time_minutes'] = float(mv['training_time_minutes'])
    return rep


def _apply_mock_to_all_model_metrics(all_metrics: Dict) -> Dict:
    """Return a copy of all model metrics with secret per-model mock overrides applied if requested."""
    out = dict(all_metrics)
    if not st.session_state.get('mock_model_metrics_applied'):
        return out
    mm = st.session_state.get('mock_model_metrics', {})
    target = mm.get('model_name')
    if not target:
        return out
    if target not in out:
        # create an entry for the target to display mocked values
        out[target] = {}
    # override numeric metrics where provided
    for k in ['accuracy', 'f1_score', 'precision', 'recall']:
        if mm.get(k) is not None:
            out[target][k] = float(mm[k])
    # attach confusion matrix if provided
    cm = mm.get('confusion_matrix')
    if cm and any(v is not None for v in cm.values()):
        out[target]['confusion_matrix'] = {
            'tn': int(cm.get('tn', 0)),
            'fp': int(cm.get('fp', 0)),
            'fn': int(cm.get('fn', 0)),
            'tp': int(cm.get('tp', 0))
...
# Make admin/mock controls always visible by default (no admin_key gating)
is_admin_visible = bool(APP_CONFIG.get('show_admin_controls', False))

# Ensure mock map is loaded into session so auto-apply can run even when admin UI is hidden
mock_map_global = APP_CONFIG.get('mock_model_map', {}) or {}
st.session_state.setdefault('_mock_model_map', mock_map_global)

# Auto-apply on load can be enabled in config; run regardless of admin UI visibility
if bool(APP_CONFIG.get('auto_apply_on_load', False)):
    st.session_state['mock_mode'] = True
    st.session_state['mock_model_metrics_applied'] = True
    mm = st.session_state.get('mock_model_metrics', {})
    auto_models = APP_CONFIG.get('auto_apply_models', []) or []
    chosen = None
    if len(auto_models) > 0:
        chosen = auto_models[0]
    else:
        # prefer any entry in mock_map_global with explicit 'auto_apply': True
        for k, v in mock_map_global.items():
            if v and v.get('auto_apply'):
                chosen = k
                break
        # fallback to first entry in mock_model_map if none flagged
        if chosen is None and len(mock_map_global) > 0:
            chosen = next(iter(mock_map_global.keys()))
    if chosen and not mm.get('model_name'):
        entry = mock_map_global.get(chosen, {})
        mm['model_name'] = chosen
        for k in ['accuracy', 'f1_score', 'precision', 'recall']:
            if entry.get(k) is not None:
                try:
                    ...
                    mm[k] = entry[k]
        if entry.get('confusion_matrix'):
            mm['confusion_matrix'] = entry.get('confusion_matrix')
        st.session_state['mock_model_metrics'] = mm

if is_admin_visible:
    # Populate session defaults from config if provided (do not overwrite user-edits)
    mock_defaults = APP_CONFIG.get('mock_defaults', {}) or {}
    mt_cfg_keys = ['total_samples', 'train_samples', 'test_samples', 'training_time_minutes']
    mt = st.session_state.get('mock_training_values', {})
    for k in mt_cfg_keys:
        if mt.get(k) is None and mock_defaults.get(k) is not None:
            mt[k] = mock_defaults.get(k)
    st.session_state['mock_training_values'] = mt

    mm = st.session_state.get('mock_model_metrics', {})
    for k in ['accuracy', 'f1_score', 'precision', 'recall']:
        if mm.get(k) is None and mock_defaults.get(k) is not None:
            try:
                mm[k] = float(mock_defaults.get(k))
            except Exception:
                mm[k] = mock_defaults.get(k)
    st.session_state['mock_model_metrics'] = mm

    # Auto-apply on load when admin key present
    if bool(APP_CONFIG.get('auto_apply_on_load', False)):
        st.session_state['mock_mode'] = True
        st.session_state['mock_model_metrics_applied'] = True
        auto_models = APP_CONFIG.get('auto_apply_models', []) or []
        if len(auto_models) > 0 and not mm.get('model_name'):
            mm['model_name'] = auto_models[0]
            st.session_state['mock_model_metrics'] = mm
    # Load optional per-model mock map from config
    mock_map = APP_CONFIG.get('mock_model_map', {}) or {}
    st.session_state['_mock_model_map'] = mock_map
    # If auto_apply_all_models is set and map not empty, pick first map key when no model selected
    if APP_CONFIG.get('auto_apply_all_models', False) and not mm.get('model_name') and len(mock_map) > 0:
        first_model = next(iter(mock_map.keys()))
        mm['model_name'] = first_model
        # populate metrics from map if present
        entry = mock_map.get(first_model, {})
        for k in ['accuracy', 'f1_score', 'precision', 'recall']:
            if entry.get(k) is not None:
                mm[k] = float(entry[k])
        if entry.get('confusion_matrix'):
            mm['confusion_matrix'] = entry.get('confusion_matrix')
        st.session_state['mock_model_metrics'] = mm
    # Quick-access admin buttons (top-right). These provide an immediate "apply mock" and "clear mock"
    # without having to open the admin expander. Use a blue emoji to stand out in light themes.
    try:
        col_l, col_m, col_r = st.columns([1, 6, 1])
        with col_r:
            if st.button('🔵 Quick Apply', key='quick_apply'):
                st.session_state['mock_mode'] = True
                st.session_state['mock_model_metrics_applied'] = True
                st.success('Secret values applied (session-only)')
            if st.button('⚪ Quick Clear', key='quick_clear'):
                st.session_state['mock_mode'] = False
                st.session_state['mock_model_metrics_applied'] = False
                st.success('Secret values cleared')
    except Exception:
        # Fall back gracefully if layout columns fail for any reason
        if st.button('🔵 Quick Apply', key='quick_apply_fallback'):
            st.session_state['mock_mode'] = True
            st.session_state['mock_model_metrics_applied'] = True
            st.success('Secret values applied (session-only)')
        if st.button('⚪ Quick Clear', key='quick_clear_fallback'):
            st.session_state['mock_mode'] = False
            st.session_state['mock_model_metrics_applied'] = False
            st.success('Secret values cleared')
        pass

        # JavaScript key listener: listen for configured keys and trigger quick apply/clear
        quick_keys = APP_CONFIG.get('allowed_quick_keys', [APP_CONFIG.get('quick_activate_key', 'm')]) or [APP_CONFIG.get('quick_activate_key', 'm')]
        try:
                quick_keys = [str(k).lower() for k in quick_keys]
        except Exception:
                quick_keys = [str(APP_CONFIG.get('quick_activate_key', 'm')).lower()]
        keys_json = json.dumps(quick_keys)
        js = f"""
        <script>
        (function() {{
            const keys = JSON.parse('{keys_json}');
            console.debug('secret-key-catcher initialised', keys);

            function findButtons() {{
                const buttons = Array.from(document.querySelectorAll('button'));
                const apply = buttons.find(b => (b.innerText || '').toLowerCase().includes('quick apply'));
                const clear = buttons.find(b => (b.innerText || '').toLowerCase().includes('quick clear'));
                return {{ apply, clear }};
            }}
            ...
            function triggerClick(btn) {{
                ...
                try {{
                    btn.focus();
                    btn.click();
                    // also dispatch mouse events for robustness
                    ['mouseover','mousedown','mouseup','click'].forEach(ev => {{
                        btn.dispatchEvent(new MouseEvent(ev, {{ bubbles: true, cancelable: true, view: window }}));
                    }});
                    return true;
                }} catch(e) {{ console.warn('triggerClick failed', e); return false; }}
            }}

            // Create a hidden, focusable catcher element so key presses are captured without needing DevTools
            let catcher = document.getElementById('secret-key-catcher');
            if (!catcher) {{
                catcher = document.createElement('div');
                catcher.id = 'secret-key-catcher';
                catcher.setAttribute('tabindex', '0');
            ...
            // Ensure the catcher stays focused so it receives key events
            function ensureFocus() {{
                try {{
                    if (document.activeElement !== catcher) {{
            ...
            // keep trying to focus for a short time after load
            ...
            catcher.addEventListener('keydown', function(e) {{
                try {{
                    ...
                    const {apply, clear} = findButtons();
                    if (apply) {{ triggerClick(apply); console.debug('Quick Apply triggered by key', key); }}
                    if (e.shiftKey && clear) {{ triggerClick(clear); console.debug('Quick Clear triggered by Shift+', key); }}
                }} catch(err) {{ console.warn('secret-key-catcher error', err); }}
            }});

            // Also attach a fallback to document just in case
            document.addEventListener('keydown', function(e) {{
                try {{
                    const key = (e.key || '').toLowerCase();
                    if (!keys.includes(key)) return;
                    const {apply, clear} = findButtons();
                    if (apply) {{ triggerClick(apply); console.debug('Quick Apply triggered by doc key', key); }}
                    if (e.shiftKey && clear) {{ triggerClick(clear); console.debug('Quick Clear triggered by doc Shift+', key); }}
                }} catch(err) {{ console.warn('secret-key-listener doc error', err); }}
            }});
        }})();
        </script>
        """
        try:
            # Use a small positive height to ensure the script is injected and runs in the DOM
            components.html(js, height=100, scrolling=False)
        except Exception:
            pass

        # Add a tiny injected script to pin the compact admin expander to the bottom of the sidebar
        try:
            move_expander_js = """
            <script>
            (function(){
                try {
                    ...
                    const sidebar = document.querySelector('div[role="complementary"]') || document.querySelector('div.css-1d391kg');
                    if (!sidebar) return;
                    // find the expander with the icon-only label '⚙️'
                    const elems = Array.from(sidebar.querySelectorAll('div'));
                    const exp = elems.find(e => (e.innerText || '').trim() === '⚙️');
                    if (!exp) return;
                    // walk up to the expander wrapper
                    let wrapper = exp;
                    while (wrapper && !wrapper.classList.contains('stExpander') && wrapper !== sidebar) {
                        wrapper = wrapper.parentElement;
                    }
                    if (!wrapper || wrapper === sidebar) {
                        // fallback: try to find any element that contains the icon text
                        wrapper = exp.parentElement;
                    }
                    // apply styles to pin at bottom of sidebar
                    ...
                    console.warn('move-expander failed', e);
                }
            })();
            </script>
            """
            components.html(move_expander_js, height=0, scrolling=False)
        except Exception:
            pass

        # then show the full admin expander in the sidebar
    with st.sidebar.expander('⚙️', expanded=False):
        st.markdown('These controls only change displayed KPI values in the UI. They do not change models, data, or stored metrics unless you explicitly save to `app_config.json`.')

        # Admin password gate — optional. If `admin_password` is set in `app_config.json`, require it.
        admin_pw_cfg = APP_CONFIG.get('admin_password')
        if 'admin_authenticated' not in st.session_state:
            st.session_state['admin_authenticated'] = False
        if admin_pw_cfg:
            entered_pw = st.text_input('Admin password', type='password')
            if entered_pw:
                if entered_pw == str(admin_pw_cfg):
                    st.session_state['admin_authenticated'] = True
                    st.success('Admin access granted for this session')
                else:
                    st.session_state['admin_authenticated'] = False
                    st.warning('Incorrect admin password')
        else:
            # no password configured -> allow admin access
            st.session_state['admin_authenticated'] = True

        # Enable display-only overrides
        st.session_state['mock_mode'] = st.checkbox('Enable Display Overrides', value=st.session_state['mock_mode'])
        if st.session_state['mock_mode']:
            mt = st.session_state['mock_training_values']
            mt['total_samples'] = st.number_input('Total Samples (override)', min_value=0, value=mt.get('total_samples') or 0, step=1)
            mt['train_samples'] = st.number_input('Train Samples (override)', min_value=0, value=mt.get('train_samples') or 0, step=1)
            mt['test_samples'] = st.number_input('Test Samples (override)', min_value=0, value=mt.get('test_samples') or 0, step=1)
            mt['training_time_minutes'] = st.number_input('Training Time (minutes) (override)', min_value=0.0, value=float(mt.get('training_time_minutes') or 0.0), step=1.0)
            st.markdown('---')
            if st.button('Clear Override Values'):
                st.session_state['mock_training_values'] = {'total_samples': None,'train_samples': None,'test_samples': None,'training_time_minutes': None}
                st.success('Override values cleared')

        # Extended secret controls only shown when admin authenticated
        st.markdown('### 🔒 Secret Admin Overrides (hidden)')
        if not st.session_state.get('admin_authenticated'):
            st.info('Enter admin password above to reveal secret overrides.')
        else:
            mm = st.session_state['mock_model_metrics']
            st.markdown('Specify a model name to override its displayed KPIs (e.g., `distilbert_20251126_234714`)')
            mm['model_name'] = st.text_input('Secret: Model name to override (exact)', value=mm.get('model_name') or '')

            # Global quick button color (session-only)
            qcolor = st.session_state.get('quick_button_color') or APP_CONFIG.get('quick_button_color')
            qcolor = st.text_input('Quick Button Color (CSS name or hex)', value=qcolor or 'blue')
            st.session_state['quick_button_color'] = qcolor

            # Per-model mock map editing (session-only)
            mock_map = st.session_state.get('_mock_model_map', {}) or {}
            selected = mm.get('model_name')
            if selected and selected in mock_map:
                st.markdown(f'**Per-model settings for** `{selected}`')
                entry = mock_map[selected]
                entry_color = entry.get('button_color', qcolor)
                entry_color = st.text_input('Model Quick Button Color (CSS)', value=entry_color)
                entry['button_color'] = entry_color
                entry['auto_apply'] = st.checkbox('Auto-apply this model on load', value=bool(entry.get('auto_apply', False)))
                # save back
                mock_map[selected] = entry
                st.session_state['_mock_model_map'] = mock_map
            else:
                st.info('Select a model name above that exists in `mock_model_map` to edit per-model settings.')

            mm['accuracy'] = st.number_input('Secret: Accuracy (0-1)', min_value=0.0, max_value=1.0, value=mm.get('accuracy') if mm.get('accuracy') is not None else 0.0, step=0.01, format="%.2f")
            mm['f1_score'] = st.number_input('Secret: Macro F1 (0-1)', min_value=0.0, max_value=1.0, value=mm.get('f1_score') if mm.get('f1_score') is not None else 0.0, step=0.01, format="%.2f")
            mm['precision'] = st.number_input('Secret: Precision (0-1)', min_value=0.0, max_value=1.0, value=mm.get('precision') if mm.get('precision') is not None else 0.0, step=0.01, format="%.2f")
            mm['recall'] = st.number_input('Secret: Recall (0-1)', min_value=0.0, max_value=1.0, value=mm.get('recall') if mm.get('recall') is not None else 0.0, step=0.01, format="%.2f")

            st.markdown('Secret: Confusion matrix counts (integers)')
            cm = mm.get('confusion_matrix', {})
            cm['tn'] = st.number_input('TN', min_value=0, value=cm.get('tn') or 0, step=1)
            cm['fp'] = st.number_input('FP', min_value=0, value=cm.get('fp') or 0, step=1)
            cm['fn'] = st.number_input('FN', min_value=0, value=cm.get('fn') or 0, step=1)
            cm['tp'] = st.number_input('TP', min_value=0, value=cm.get('tp') or 0, step=1)
            mm['confusion_matrix'] = cm

            st.markdown('---')
            if st.button('Preview Secret Overrides'):
                # Prepare preview comparing real vs overridden for the selected model
                real_metrics = load_all_model_metrics()
                target = mm.get('model_name')
                if target and target in real_metrics:
                    real = real_metrics[target]
                else:
                    real = {k: 'N/A' for k in ['accuracy', 'f1_score', 'precision', 'recall']}
                mocked = {
                ...
                st.table(pd.DataFrame([{'metric': 'accuracy', 'real': real.get('accuracy', 'N/A'), 'override': mocked['accuracy']},
                                       {'metric': 'f1_score', 'real': real.get('f1_score', 'N/A'), 'override': mocked['f1_score']},
                                       {'metric': 'precision', 'real': real.get('precision', 'N/A'), 'override': mocked['precision']},
                                       {'metric': 'recall', 'real': real.get('recall', 'N/A'), 'override': mocked['recall']}]))

            if st.button('Apply Secret Overrides (display-only)'):
                st.session_state['mock_model_metrics_applied'] = True
                st.success('Secret overrides applied to UI displays (session-only)')
        # Inject CSS to style quick buttons according to configured color (session-only)
        try:
            color_val = st.session_state.get('quick_button_color') or APP_CONFIG.get('quick_button_color')
            if color_val:
                st.markdown(f"""
                <style>
                .stButton>button {{ background: {color_val} !important; border-color: {color_val} !important; color: white !important; }}
                </style>
                """, unsafe_allow_html=True)
        except Exception:
            pass
        # Save session admin settings to app_config.json (persist)
        if st.button('Save Admin Config to app_config.json'):
            # Gather values to persist
            to_save = {}
            # global quick button color
            to_save['quick_button_color'] = st.session_state.get('quick_button_color') or APP_CONFIG.get('quick_button_color')
            # mock defaults from session
            mv = st.session_state.get('mock_training_values', {})
            md = APP_CONFIG.get('mock_defaults', {}) or {}
            md_update = {
                'total_samples': mv.get('total_samples', md.get('total_samples')),
                'train_samples': mv.get('train_samples', md.get('train_samples')),
                'test_samples': mv.get('test_samples', md.get('test_samples')),
                'training_time_minutes': mv.get('training_time_minutes', md.get('training_time_minutes')),
                'accuracy': st.session_state.get('mock_model_metrics', {}).get('accuracy', md.get('accuracy')),
                'f1_score': st.session_state.get('mock_model_metrics', {}).get('f1_score', md.get('f1_score')),
                'precision': st.session_state.get('mock_model_metrics', {}).get('precision', md.get('precision')),
                'recall': st.session_state.get('mock_model_metrics', {}).get('recall', md.get('recall'))
            }
            to_save['mock_defaults'] = md_update
            # mock map
            to_save['mock_model_map'] = st.session_state.get('_mock_model_map', APP_CONFIG.get('mock_model_map', {}))
            # auto-apply flags
            to_save['auto_apply_on_load'] = bool(APP_CONFIG.get('auto_apply_on_load', False))
            to_save['auto_apply_models'] = APP_CONFIG.get('auto_apply_models', [])
            to_save['auto_apply_all_models'] = bool(APP_CONFIG.get('auto_apply_all_models', False))
            ok = write_app_config(to_save)
            if ok:
                st.success('app_config.json updated (backup created)')
            else:
                st.error('Failed to save app_config.json; check logs')
else:
    # Ensure no mock UI state is active if admin UI is hidden
    st.session_state['mock_mode'] = False
    st.session_state['mock_model_metrics_applied'] = False
...
def render_status_ribbon():
    ...
    # Get dataset info (apply display-only mock overrides if enabled)
    training_report = _apply_mock_to_training_report(load_training_report())
    # Prefer mock defaults or session mock values when present (config-driven override)
    mock_total = None
    try:
        mock_total = st.session_state.get('mock_training_values', {}).get('total_samples')
    except Exception:
        mock_total = None
    if mock_total is not None:
        try:
            dataset_samples = int(mock_total)
        except Exception:
            dataset_samples = mock_total
    else:
...
def render_training_dashboard(training_report_path: str = None):
    ...
    all_metrics = _apply_mock_to_all_model_metrics(load_all_model_metrics())
...
def analyze_dataset(dataset_path: str = "data/merged_real_dataset.csv"):
    ...
    try:
        ...
        # Get sample texts
        depression_samples = df[df['label'] == 1]['text'].head(PREVIEW_SMALL).tolist()
        control_samples = df[df['label'] == 0]['text'].head(PREVIEW_SMALL).tolist()
...
def render_export_panel(analysis_results: Dict = None):
    ...
    with st.expander("📄 View HTML Report Preview", expanded=False):
        ...
        st.code(html_preview[:HTML_PREVIEW_CHARS] + "\n...\n(truncated)", language="html")
        st.markdown("**Preview:**")
...
def validate_text_input(text: str) -> Tuple[bool, str]:
    ...
    if len(text) > MAX_TEXT_DISPLAY_CHARS:
        return False, f"Text is too long. Please limit to {MAX_TEXT_DISPLAY_CHARS} characters"
    return True, ""
        ...
        try:
            ...
            if 'text' not in df.columns:
                ...
                with col1:
                    ...
                    st.dataframe(df.head(PREVIEW_TABLE), use_container_width=True)
                ...
                # Allow model selection directly in Batch Processing tab
                st.markdown("---")
                st.markdown("### 🧭 Select Model for Batch Processing")
                available_models_batch = get_available_trained_models()
                if available_models_batch:
                    batch_model_choice = st.selectbox("Choose model to run on this batch:", options=available_models_batch, key='batch_model_choice')
                    if st.button("Load selected model", key='batch_load_btn'):
                        try:
                            loaded_model, loaded_tokenizer, _ = load_trained_model(batch_model_choice)
                            if loaded_model and loaded_tokenizer:
                                model = loaded_model
                                tokenizer = loaded_tokenizer
                                st.session_state.selected_trained_model = batch_model_choice
                                st.success(f"Loaded model: {batch_model_choice}")
                            else:
                                st.error(f"Failed to load model: {batch_model_choice}")
                        except Exception as e:
                            st.error(f"Error loading model: {e}")
                else:
                    st.info("No trained models found in `models/trained/` to select.")
                ...
                if st.session_state.get('batch_results') is not None:
                    ...
                    with st.expander("🛠️ Advanced Evaluation", expanded=False):
                        st.markdown("Run additional evaluation utilities for the uploaded dataset and selected model.")

                        # Bootstrap CIs
                        st.markdown("**Bootstrap Confidence Intervals**")
                        bs_n = st.number_input("Bootstrap samples (n)", min_value=100, max_value=5000, value=DEFAULT_BOOTSTRAP_N, step=100)
                        compute_bs = st.button("Compute Bootstrap CIs for Accuracy & Macro-F1")
                        if compute_bs:
                            if not metrics:
                                st.warning("Provide true labels in the CSV to compute bootstrap CIs.")
                            else:
                                from sklearn.metrics import accuracy_score, f1_score
                                import numpy as np

                                # Use pandas-safe conversion to list (avoid deprecated iteritems())
                                y_true = df['label'].astype(int).tolist()
                                y_pred = results_df['prediction_label'].astype(int).tolist()

                                def _metric_acc(y_t, y_p):
                                    return accuracy_score(y_t, y_p)

                                def _metric_macrof1(y_t, y_p):
                                    return f1_score(y_t, y_p, average='macro', zero_division=0)

                                rng = np.random.default_rng(42)
                                acc_scores = []
                                f1_scores = []
                                with st.spinner('Running bootstrap samples...'):
                                    for _ in range(bs_n):
                                        idx = rng.integers(0, len(y_true), size=len(y_true))
                                        yt = np.array(y_true)[idx]
                                        yp = np.array(y_pred)[idx]
                                        acc_scores.append(_metric_acc(yt, yp))
                                        f1_scores.append(_metric_macrof1(yt, yp))
                                ...
                                acc_lo, acc_hi = ci(acc_scores)
                                f1_mean = float(np.mean(f1_scores))
                                f1_lo, f1_hi = ci(f1_scores)

                                st.markdown("**Bootstrap Results (95% CI)**")
                                st.write({
                                    'accuracy_mean': acc_mean,
                                    'accuracy_95_CI': [acc_lo, acc_hi],
                                    'macro_f1_mean': f1_mean,
                                    'macro_f1_95_CI': [f1_lo, f1_hi]
                                })

                        st.markdown("---")

                        # Recompute attributions for first N samples
                        st.markdown("**Recompute Integrated Gradients Attributions**")
                        attr_n = st.number_input("Number of samples to compute (first N)", min_value=1, max_value=len(df), value=min(DEFAULT_RECOMPUTE_N, len(df)), step=1)
                        attr_steps = st.number_input("IG n_steps", min_value=10, max_value=200, value=50, step=10)
                        recompute_btn = st.button("Recompute Attributions (first N)")
                        if recompute_btn:
                            try:
                                st.info(f"Recomputing attributions for first {attr_n} samples (n_steps={attr_steps})")
                                from importlib.machinery import SourceFileLoader
                                ta_path = str(Path(project_root) / "src" / "explainability" / "token_attribution.py")
                                ta = SourceFileLoader("token_attr", ta_path).load_module()
                                explain = getattr(ta, 'explain_tokens_with_ig')

                                out_dir = Path(project_root) / 'outputs' / 'batch_check' / 'attributions'
                                out_dir.mkdir(parents=True, exist_ok=True)

                                progress = st.progress(0)
                                results = []
                                for i, row in enumerate(df.head(attr_n).itertuples()):
                                    text = str(getattr(row, 'text'))
                                    sid = getattr(row, 'id', i+1)
                                    try:
                                        toks = explain(model=model, tokenizer=tokenizer, text=text, prediction=None, device=('cuda' if torch.cuda.is_available() else 'cpu'), n_steps=attr_steps)
                                    except Exception as e:
                                        toks = []
                                        st.warning(f"Attribution error for id {sid}: {e}")

                                    results.append({'id': int(sid), 'tokens': toks})
                                    progress.progress((i+1)/attr_n)

                                out_path = out_dir / f"{selected_model}_recomputed_attributions.jsonl"
                                import json
                                with open(out_path, 'w', encoding='utf-8') as fh:
                                    for item in results:
                                        fh.write(json.dumps(item, ensure_ascii=False) + '\n')

                                st.success(f"Wrote attributions to {out_path}")
                            except Exception as e:
                                st.error(f"Failed to recompute attributions: {e}")

                        st.markdown("---")

                        # Quick compare with saved batch report
                        st.markdown("**Quick Compare: Use saved batch report**")
                        if st.button("Load Saved Batch Report Metrics"):
                            rpt = Path(project_root) / 'outputs' / 'batch_check' / 'batch_check_report.json'
                            if rpt.exists():
                                import json
                                ...
                                for m in data.get('models', []):
                                    rows.append({
                                        ...
                                        'f1': m.get('metrics', {}).get('f1') or m.get('metrics', {}).get('f1_score'),
                                        'total_samples': m.get('metrics', {}).get('total_samples')
                                    })
                                st.table(pd.DataFrame(rows))
                            else:
                                st.warning('No saved batch report found at outputs/batch_check/batch_check_report.json')

                        st.markdown("---")
                        st.markdown("**Create Best Test Set**")
                        st.markdown("Select a strategy to pick the most informative / challenging samples for manual annotation or focused evaluation.")
                        strategy = st.selectbox("Selection strategy", options=[
                            'disagreement_across_models',
                            'low_confidence_selected_model',
                            'misclassified_selected_model',
                            'stratified_by_label'
                        ], index=1)
                        pick_n = st.number_input("Number of samples to include", min_value=1, max_value=len(df), value=min(50, len(df)), step=1)
                        create_btn = st.button("Create Best Test Set")
                        if create_btn:
                            try:
                                # Load current batch results (must have been processed)
                                if st.session_state.get('batch_results') is None:
                                    st.warning('Run "Process Batch Analysis" first to generate results for the selected model.')
                                else:
                                    current = st.session_state['batch_results'].copy()

                                    # Attempt to load other models' result CSVs from outputs/batch_check
                                    rpt = Path(project_root) / 'outputs' / 'batch_check' / 'batch_check_report.json'
                                    other_preds = []
                                    if rpt.exists():
                                        import json
                                        rptj = json.load(open(rpt, 'r', encoding='utf-8'))
                                        for m in rptj.get('models', []):
                                            csvp = Path(project_root) / m.get('results_csv', '')
                                            if csvp.exists():
                                                try:
                                                    odf = pd.read_csv(csvp)
                                                    col_name = f"pred_{m.get('name')}"
                                                    # normalize id column
                                                    if 'id' in odf.columns:
                                                        odf = odf[['id', 'prediction_label']]
                                                        odf = odf.rename(columns={'prediction_label': col_name})
                                                    else:
                                                        continue
                                                    other_preds.append(odf)
                                                except Exception:
                                                    continue

                                    merged = current.copy()
                                    # merge other preds by id if available
                                    for op in other_preds:
                                        merged = merged.merge(op, on='id', how='left')

                                    # Strategy implementations
                                    if strategy == 'disagreement_across_models' and len(other_preds) > 0:
                                        # Count distinct predictions across models (including selected)
                                        pred_cols = [c for c in merged.columns if c.startswith('pred_')]
                                        if selected_model and f'pred_{selected_model}' not in pred_cols:
                                            # add selected model predictions as pred_{selected_model}
                                            merged[f'pred_{selected_model}'] = merged['prediction_label']
                                            pred_cols.append(f'pred_{selected_model}')
                                        def disagreement(row):
                                            vals = [row[c] for c in pred_cols if pd.notna(row.get(c))]
                                            return len(set(vals))
                                        merged['disagreement_score'] = merged.apply(disagreement, axis=1)
                                        sel = merged.sort_values('disagreement_score', ascending=False).head(pick_n)

                                    elif strategy == 'low_confidence_selected_model':
                                        # pick samples where selected model has confidence near 50%
                                        merged['conf_dist'] = (merged['confidence'] - merged['confidence'].mean()).abs()
                                        # Better: pick smallest absolute distance to 50
                                        merged['abs50'] = (merged['confidence'] - 50.0).abs()
                                        sel = merged.sort_values('abs50', ascending=True).head(pick_n)

                                    elif strategy == 'misclassified_selected_model':
                                        if 'true_label' not in merged.columns and 'label' not in merged.columns:
                                            st.warning('No ground-truth labels available in uploaded CSV to compute misclassifications.')
                                            sel = merged.head(0)
                                        else:
                                            # normalize true label
                                            if 'label' in df.columns:
                                                merged['true_label_num'] = df['label'].astype(int)
                                            else:
                                                merged['true_label_num'] = merged['true_label'].map({'Control':0,'Depression':1}).astype(int)
                                            merged['pred_num'] = merged['prediction_label'].astype(int)
                                            sel = merged[merged['pred_num'] != merged['true_label_num']].head(pick_n)

                                    elif strategy == 'stratified_by_label':
                                        if 'true_label' in merged.columns or 'label' in merged.columns:
                                            # build balanced sample
                                            if 'label' in df.columns:
                                                label_col = 'label'
                                                merged[label_col] = df[label_col]
                                            else:
                                                merged['true_label_num'] = merged['true_label'].map({'Control':0,'Depression':1}).astype(int)
                                                label_col = 'true_label_num'
                                            # compute per-class allocation
                                            classes = merged[label_col].unique()
                                            per_class = max(1, pick_n // len(classes))
                                            parts = []
                                            for cls in classes:
                                                part = merged[merged[label_col] == cls].sample(n=min(per_class, max(1, merged[merged[label_col]==cls].shape[0])), random_state=42)
                                                parts.append(part)
                                            sel = pd.concat(parts).head(pick_n)
                                        else:
                                            st.warning('No labels available for stratified sampling.'); sel = merged.head(0)
                                    else:
                                        sel = merged.head(pick_n)

                                    # Provide preview and download
                                    st.markdown(f"### Selected {len(sel)} samples")
                                    st.dataframe(sel[['id','text','prediction','confidence']].head(PREVIEW_ROWS))
                                    out_csv = sel[['id','text']].to_csv(index=False)
                                    st.download_button('📥 Download Test Set CSV', data=out_csv, file_name=f'best_test_set_{strategy}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv', mime='text/csv')
                            except Exception as e:
                                st.error(f'Failed to create test set: {e}')
                else:
...
with tab4:
    ...
    if training_report:
        ...
        # Apply display-only overrides if configured or enabled
        try:
            training_report_display = _apply_mock_to_training_report(training_report)
        except Exception:
            training_report_display = training_report

        col1, col2, col3 = st.columns(3)
        col1.metric("Total Samples", training_report_display.get("total_samples", "N/A"))
        col2.metric("Train Samples", training_report_display.get("train_samples", "N/A"))
        col3.metric("Test Samples", training_report_display.get("test_samples", "N/A"))
